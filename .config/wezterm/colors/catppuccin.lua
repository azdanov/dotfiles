...
function M.apply_to_config(c, opts)
	...
	if opts.sync then
		flavor = select_for_appearance(wezterm.gui.get_appearance(), {
			dark = o.sync_flavors.dark,
			light = o.sync_flavors.light,
		})
		...
		c.color_scheme = mappings[flavor]
		c.command_palette_bg_color = colors[flavor].crust
		c.command_palette_fg_color = colors[flavor].text
	end

	local border_color = colors[flavor].surface2
	if flavor == "latte" then
		border_color = colors[flavor].overlay2
	end

	local window_frame = {
		-- active_titlebar_bg = colors[flavor].crust,
		-- active_titlebar_fg = colors[flavor].text,
		-- active_titlebar_border_bottom = colors[flavor].crust,
		-- inactive_titlebar_bg = colors[flavor].crust,
		-- inactive_titlebar_fg = colors[flavor].text,
		-- inactive_titlebar_border_bottom = colors[flavor].text,
		-- button_fg = colors[flavor].text,
		-- button_bg = colors[flavor].base,
