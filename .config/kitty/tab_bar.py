...
# Cache TTL in seconds. Adjust as needed.
CACHE_TTL = 60
_battery_cache = {"timestamp": 0, "result": []}
...
def get_battery_cells() -> list:
    try:
        if sys.platform == "darwin":
            ...
            if len(lines) < 2:
                raise ValueError("No battery info available.")
            battery_line = lines[1]
            m = re.search(
                r"(\d+)%.* (discharging|charging|finishing charge|charged)",
                battery_line,
                re.IGNORECASE,
            )
            if m:
                ...
                raise ValueError("Unexpected battery info: " + battery_line)
        else:
        ...
        percent_cell = (bat_text_color, f"{percent}% ")
        icon_cell = (icon_color, icon)
        cells = [percent_cell, icon_cell]
        return cells
    except Exception:
        return []


def get_cached_battery_cells() -> list:
    global _battery_cache
    current_time = time.time()
    # Check if the cached result is still valid
    # Otherwise, update the cache
    if (current_time - _battery_cache["timestamp"]) < CACHE_TTL:
        return _battery_cache["result"].copy()

    cells = get_battery_cells()
    _battery_cache["timestamp"] = current_time
    _battery_cache["result"] = cells
    return cells.copy()
...
def draw_tab(
    ...
    cells = get_cached_battery_cells()
